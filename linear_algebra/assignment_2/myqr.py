...
def qr_algo(A):
    ...
    for i in range(cols-1):
        ...
        norm_value =  np.linalg.norm(col_i)
        if(norm_value != 0): 
        ...
        #p_matrix = np.subtract(np.eye(rows) , 2 * np.dot(v_matrix_transpose,v_matrix))
        if i == 0:
...
def multiply_matrix_list(list_m):
    multiply = list_m[0]
    i = 1
    while(i<len(list_m)):
        multiply = multiply_matrices(multiply , list_m[i])
        i = i + 1
    return multiply
...
def eig_by_qr(a):
    ...
    multiplication_of_qs = multiply_matrix_list(q_s_multiply_Wale)
