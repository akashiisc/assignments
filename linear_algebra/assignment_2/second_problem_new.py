...
file_name = "./LA_IP_OP/input_second_problem/mnist_train.csv"
...
task_plot_graph(reconstruction_error_map, True, output_plot_dir + "problem_2_task_5.png", "Dimension",
                "Reconstruction error", "Reconstrunction error")
print_barrier(file_to_write)
print_beautifully(reconstruction_error_map, "Reconstruction Error", "Dimentionality Reduction", "Reconstruction error",
                  True, file_to_write, "map")
print_barrier(file_to_write)
...
reduced_dimention_matrix = opr_multiply(matrix , top_m_eigen_vectors)
reduced_dimention_test_matrix = opr_multiply(test_matrix , top_m_eigen_vectors)
reduced_dimention_matrix = np.array(reduced_dimention_matrix).real
reduced_dimention_test_matrix = np.array(reduced_dimention_test_matrix).real
...
# test_matrix_converted_mean_vector_to_matrix = []
# for j in range(len(matrix)):
#     a = copy(mean_vector_of_reduced)
#     test_matrix_converted_mean_vector_to_matrix.append(a)
converted_minus_mean_matrix_test_data = opr_subtract(reduced_dimention_test_matrix, converted_minus_mean_matrix)

correctness_difference_vector = []
for i in range(len(converted_minus_mean_matrix_test_data)):
    squared_distance_from_all = {}
    for j in range(10000):
        #squared_distance = calculate_square_distance(converted_minus_mean_matrix_test_data , i , j)
        squared_distance = calculate_square_distance_two_matrices(converted_minus_mean_matrix_test_data , converted_minus_mean_matrix , i , j )
    ...
    top_m_points = take_top_m_points(sorted_squared_distance_from_all , k_value)
    ...
    correctness_difference_vector.append(majority_element - test_label_matrix[i])
